...
for day in range(days):
    ...
    wordle_num = input("wordle number: ")
    ...
    with open(f"new-sw-possibilities-{todays_word}-{wordle_num}.txt", "wt") as nsfile:
        for match in current_matches:
            nsfile.write(match + '\n')

    wordlist_file.append(f"new-sw-possibilities-{todays_word}-{wordle_num}.txt")
    print(f'used {wordlist_file[0]} for {wordlist_file[1]}')
    wordlist_file.pop(0)
